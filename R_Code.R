...
# TODO: Test interactions by male/female
...
# Isn't monotonic. Cut arbitrarily into 10 groups.
...
# Note: Geom_text is causing trouble with the sorting.
...
prediction <- function(newdata) {
  ...
  preds <- predict.glm(object = fit, type = "response", newdata)
  return(ifelse(preds>0.5, 1, 0))
}

sum(prediction(test)==test$over_50k)/9768
                # Final Accuracy: 84.3%
